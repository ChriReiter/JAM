...
class UserGroupViewSet(viewsets.ViewSet):
    ...
    def list(self, request, user_pk):
        ...
        '''         
        user = get_object_or_404(User,pk=user_pk)
        return Response([{"name":group.name,"url":"http://localhost:8000/api/users/%s/groups/%s" % (user.pk,group.pk)} for group in user.groups.all()])
